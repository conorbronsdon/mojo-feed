...
from feed import line_col, parse_error, parse_feed, parse_date
from feed.jsonfeed import parse_json_feed
from feed.xml_parser import XmlPullParser
...
# --------------------------------------------------------------------------
# Integration — real malformed feed inputs report hand-verified positions.
# --------------------------------------------------------------------------


def test_integration_strict_mismatched_end_tag_position() raises:
    # "<rss>\n<item>\nx</rss>\n</item>\n</rss>": the offending "</rss>"
    # starts at byte 14 — line 3 (newlines at 5 and 12), column
    # 14-12 = 2. Line 3 is "x</rss>".
    with assert_raises(
        contains=(
            "mismatched end tag </rss>, expected </item>"
            " at line 3, column 2: 'x</rss>'"
        )
    ):
        _ = parse_feed("<rss>\n<item>\nx</rss>\n</item>\n</rss>", strict=True)
...
def test_integration_unterminated_attribute_value_position() raises:
    ...
    var parser = XmlPullParser('<rss version="2.0\n')
    with assert_raises(
        contains=(
            "unterminated attribute value"
            " at line 1, column 14: '<rss version=\"2.0'"
        )
    ):
        _ = parser.next_event()


def test_integration_json_feed_position() raises:
    # '{\n  "title": bad\n}': the offending 'b' is at byte 13 — line 2,
    # column 13-1 = 12. Line 2 trims to '"title": bad'.
    with assert_raises(
        contains="expected number at line 2, column 12: '\"title\": bad'"
    ):
        _ = parse_json_feed('{\n  "title": bad\n}')


def test_integration_date_position() raises:
    # "Mon, xx Oct 2024": the parser expects the day number at byte 5 —
    # line 1, column 6 of the (trimmed) date string.
    with assert_raises(
        contains=(
            "expected digits in date at line 1, column 6: 'Mon, xx Oct 2024'"
        )
    ):
        _ = parse_date("Mon, xx Oct 2024")
